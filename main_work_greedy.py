...
class CollectCoinThread(threading.Thread):
    ...
    @staticmethod
    def create_matrix(start_pos,coin_pos, monsters_pos, fires_pos): #

        tem_pos=coin_pos
        tem_pos.insert(0,start_pos)
        tem_dif_y=[]
        for i in range(len(coin_pos)):
            tem_dif_x=[]
            for j in range(len(coin_pos)):
                if (i==j):
                    tem_dif_x.append(-1)
                else:
                    tem_dif_x.append( abs(coin_pos[i][0]-coin_pos[j][0])+abs(coin_pos[i][1]-coin_pos[j][1]) )
            tem_dif_y.append(tem_dif_x)

        print("coin_pos=",tem_dif_y)
        return tem_dif_y
    ...
    def run(self):
        ...
        coin_pos, monsters_pos, fires_pos = position_func_v3()
        tsp= CollectCoinThread.create_matrix(start_pos,coin_pos, monsters_pos, fires_pos)
        print("tsp=", tsp)
        route=CollectCoinThread.findMinRoute(tsp)
        print("route :", route)

        for i in range(0,len(route)):
            print("coin_pos[i]=",coin_pos[i])
        #    app.start_char_animation(lvl_num, [float(random.uniform(0, 1)), float(random.uniform(0, 1))])
            app.start_char_animation(lvl_num, coin_pos[i])
            time.sleep(0.4)
